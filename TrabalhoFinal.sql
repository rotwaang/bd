...
BEGIN
    DECLARE uuid_simulacao VARCHAR(36);
    SET uuid_simulacao = UUID(); -- Garante unicidade absoluta

    -- Transação 1
    START TRANSACTION;
    UPDATE Alunos 
    SET email = CONCAT('deadlock1_', uuid_simulacao, '@teste.com') 
    WHERE id = 1;

    DO SLEEP(2); -- Cria janela para deadlock
    ...
    -- Transação 2 (simulação concorrente)
    START TRANSACTION;
    UPDATE Alunos 
    SET email = CONCAT('deadlock2_', uuid_simulacao, '@teste.com') 
    WHERE id = 2;
    ...
    INSERT INTO Logs_Testes (equipe_id, questao_id, evento) 
    VALUES (equipeId, 8, 'Deadlock simulado com UUID único.');
END$$
DELIMITER ;
